    ...
    public partial class ManagePlaylist : System.Web.UI.Page
    {
        ...
        protected void DeleteTrack_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (PlayList.Rows.Count == 0)
                {
                    MessageUserControl.ShowInfo("Missing Data", "You require a playlist from which to select song(s) to remove.");
                }
                else
                {
                    ...
                    if(rowsSelected == 0)
                    {
                        MessageUserControl.ShowInfo("Missing Data", 
                            "You require to select at least one song(s) to remove.");
